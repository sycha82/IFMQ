...
-- PRE03 매핑 등록 시점부터 라이프사이클 시작. PENDING 상태 없음.
-- (PENDING 검증은 wcs-app 의 inf.if_msg_log 를 조회하여 처리)
CREATE TABLE IF NOT EXISTS biz.wcs_eqp_pallet_mapping_m (
    eqp_pallet_id   VARCHAR(40)  PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS ix_wcs_eqp_pallet_mapping_m_pallet_id
    ON biz.wcs_eqp_pallet_mapping_m (pallet_id);
